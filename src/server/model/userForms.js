          ...
          connection.query(FORM_CREATE.CREATE_NEW_FORM_ENTRY, createNewFormEntryInput, (err2, result) => {
            ...
            if (result) {
              connection.query(FORM_READ.USERFORMS_SELECT_BY_ROWID, { id: result.insertId }, (err3, rows) => {
                ...
                connection.query(FORM_CREATE.CREATE_NEW_FORM_DATA_ENTRY, formDataInput, (err4) => {
                  if (err4) cb(err4, null);
                  connection.query(FORM_CREATE.CREATE_NEW_FORM_DATA_EXTRA_INFO_ENTRY, formDataExtraInfoInput, (err5) => {
                    ...
                    parallel([relationsModel, tripsModel, documentsModel], (asyncParallelError) => {
              ...
              sqlConnPool.getConnection((err, connection) => {
                ...
                connection.beginTransaction((err1) => {
                  ...
                  connection.query(FORM_READ.USERFORMS_SELECT_BY_FORMID_USERID, [sanitizedInput.uniqueId, currentUser.id], (err3, rows) => {
                    ...
                    connection.query(FORM_UPDATE.UPDATE_NEW_FORM_DATA_ENTRY, [formDataInput, formUID], (err4) => {
                      if (err4) cb(err4, null);
                      connection.query(FORM_UPDATE.UPDATE_NEW_FORM_DATA_EXTRA_INFO_ENTRY, [formDataExtraInfoInput, formUID], (err5) => {
                        if (err5) cb(err5, null);
              ...
              if (result) {
                connection.query(FORM_READ.USERFORMS_SELECT_BY_ROWID, { id: result.insertId }, (err3, rows) => {
                  ...
                  connection.query(FORM_CREATE.CREATE_NEW_FORM_DATA_ENTRY, formDataInput, (err4) => {
                    ...
                    connection.query(FORM_CREATE.CREATE_NEW_FORM_DATA_EXTRA_INFO_ENTRY, formDataExtraInfoInput, (err5) => {
                      if (err5) cb(err5, null);
                      connection.query(FORM_UPDATE.UPDATE_FORM_SUBMIT_BY_FORMID_USERID, [{ processingStatus: formProcessingStatus.SUBMITTED, status: SUBMIT }, formUID, currentUser.id], (err6) => {
                        if (err6) cb(err6, null);
                        // commit the transaction here
